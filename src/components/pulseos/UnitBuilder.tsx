...
export function UnitBuilder({
  ...
  if (selectedUnit) {
    ...
    const sc = statusConfig[selectedUnit.status];

    return (
      <div className="space-y-6">
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-sm">
          <button onClick={() => onSelectUnit(null)} className="font-semibold text-navy-700 transition-colors hover:text-gold-700">
            All Units
          </button>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex-1">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xl font-bold text-navy-900 transition-colors focus:border-gold-400 focus:outline-none focus:ring-2 focus:ring-gold-100"
                ...
                <textarea
                  ...
                  className="mt-3 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-600 transition-colors focus:border-gold-400 focus:outline-none focus:ring-2 focus:ring-gold-100"
                  rows={2}
                />
              </div>
              <div className="flex flex-col items-end gap-2">
                ...
                <span className="text-xs font-medium text-slate-400">Version {selectedUnit.version}</span>
              </div>
            </div>

            {/* UbD completeness bar */}
            <div className="mt-6 rounded-xl bg-softgray p-5">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wide text-navy-700">UbD Completeness</span>
                <span className="text-xs font-bold text-navy-900">{completeness.stage}/3 Stages</span>
              </div>
              <div className="flex gap-2">
                {[1, 2, 3].map(s => (
                  <div key={s} className={`h-2.5 flex-1 rounded-full transition-all duration-500 ${s <= completeness.stage ? 'bg-gradient-to-r from-gold-400 to-gold-500' : 'bg-slate-200'}`} />
                ))}
              </div>
                ...
                <button
                  ...
                  className={`rounded-lg px-3.5 py-1.5 text-xs font-semibold transition-all ${
                    selectedUnit.status === s
                      ...
                      : 'border border-slate-200 text-slate-600 hover:border-navy-300 hover:bg-slate-50'
                  }`}
                >
                  {s.replace('_', ' ')}
                </button>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Drag units to reorder. Click any card to open the full UbD editor.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6">
          <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-navy-50 opacity-50" />
          <div className="relative">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowProgramForm(!showProgramForm)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-semibold text-navy-700 transition-colors hover:border-gold-400 hover:text-gold-700">
                <Plus className="h-3.5 w-3.5" /> New
              </button>
            </div>
            {showProgramForm && (
              <div className="mb-4 space-y-2 rounded-xl bg-softgray p-4">
                <input value={newProgramTitle} onChange={e => setNewProgramTitle(e.target.value)} placeholder="Program title" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-gold-400 focus:outline-none" />
                <input value={newProgramSubject} onChange={e => setNewProgramSubject(e.target.value)} placeholder="Subject (optional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-gold-400 focus:outline-none" />
                <input value={newProgramGrade} onChange={e => setNewProgramGrade(e.target.value)} placeholder="Grade band (optional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-gold-400 focus:outline-none" />
                <button onClick={createProgram} className="w-full rounded-lg bg-navy-900 py-2 text-xs font-semibold text-white transition-colors hover:bg-navy-800">Create Program</button>
              </div>
            )}
            {programs.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {programs.map(p => (
                  <div key={p.id} className="flex items-center gap-2.5 rounded-lg border border-slate-100 px-3 py-2 text-sm transition-colors hover:border-slate-200 hover:bg-slate-50">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6">
          <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-gold-50 opacity-50" />
          <div className="relative">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowCourseForm(!showCourseForm)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-semibold text-navy-700 transition-colors hover:border-gold-400 hover:text-gold-700">
                <Plus className="h-3.5 w-3.5" /> New
              </button>
            </div>
            {showCourseForm && (
              <div className="mb-4 space-y-2 rounded-xl bg-softgray p-4">
                <select value={newCourseProgramId} onChange={e => setNewCourseProgramId(e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-gold-400 focus:outline-none">
                  <option value="">Select program...</option>
                  {programs.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
                </select>
                <input value={newCourseTitle} onChange={e => setNewCourseTitle(e.target.value)} placeholder="Course title" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-gold-400 focus:outline-none" />
                <button onClick={createCourse} className="w-full rounded-lg bg-navy-900 py-2 text-xs font-semibold text-white transition-colors hover:bg-navy-800">Create Course</button>
              </div>
            )}
            {courses.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {courses.map(c => {
                  ...
                  return (
                    <div key={c.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 text-sm transition-colors hover:border-slate-200 hover:bg-slate-50">
      ...
      <div>
        <h3 className="mb-4 text-sm font-bold text-navy-900">All Units</h3>
        {units.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-white p-16 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-navy-50">
              <Brain className="h-8 w-8 text-navy-300" />
            </div>
            <p className="text-sm font-medium text-slate-500">No UbD units yet.</p>
            {courses.length > 0 ? (
              <button onClick={() => createUnit(courses[0].id)} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-navy-900 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-navy-800 hover:shadow-lg">
...
function UnitCardInner({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 transition-all hover:-translate-y-1 hover:border-gold-300 hover:shadow-xl">
      ...
      <div className="relative">
        <div className="mb-3 flex items-center justify-between">
          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${statusColors[unit.status]}`}>
            <span className={`h-1.5 w-1.5 rounded-full ${statusDots[unit.status]}`} />
            {unit.status.replace('_', ' ')}
          </span>
          <div className="flex items-center gap-1">
            {[1, 2, 3].map(s => (
              <div key={s} className={`h-1.5 w-6 rounded-full transition-colors ${s <= comp.stage ? 'bg-gold-500' : 'bg-slate-200'}`} />
            ))}
          </div>
        </div>
        <h4 className="text-base font-bold text-navy-900 transition-colors group-hover:text-gold-700">{unit.title}</h4>
        <p className="mt-1.5 text-xs text-slate-500">{program?.title ?? '—'} / {course?.title ?? '—'}</p>
...
function Stage1Editor({ unit, onSave }: { unit: UbDUnit; onSave: (u: UbDUnit) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-navy-50 to-navy-100 p-5">
        <div className="flex items-center gap-2">
          <Target className="h-5 w-5 text-navy-700" />
          <h3 className="text-sm font-bold text-navy-900">Stage 1 — Desired Results</h3>
        </div>
        <p className="mt-1.5 text-xs text-slate-600">Define what learners should understand, know, and be able to do. This is the foundation of backward design.</p>
      </div>
...
function Stage2Editor({ unit, onSave }: { unit: UbDUnit; onSave: (u: UbDUnit) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-gold-50 to-gold-100 p-5">
        <div className="flex items-center gap-2">
          <FileText className="h-5 w-5 text-navy-700" />
          <h3 className="text-sm font-bold text-navy-900">Stage 2 — Evidence</h3>
        </div>
        <p className="mt-1.5 text-xs text-slate-600">Design assessments and performance tasks that provide evidence of understanding. This is where you prove learning happened.</p>
      </div>
...
function Stage3Editor({ unit, onSave }: { unit: UbDUnit; onSave: (u: UbDUnit) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 p-5">
        <div className="flex items-center gap-2">
          <BookOpen className="h-5 w-5 text-navy-700" />
          <h3 className="text-sm font-bold text-navy-900">Stage 3 — Learning Plan</h3>
        </div>
        <p className="mt-1.5 text-xs text-slate-600">Plan the learning experiences, sequence, and resources that will get learners to the desired results. This is the instructional roadmap.</p>
      </div>
